...
def about():
    session["ret"] = url_for("about")
    log = "Logout" if session.get("login") else "Login"
    return render_template("about.html", log=log, portfolio=session.get("portfolio") or get_portfolio())

# Essential
@app.route("/essential")
def essential():
    session["ret"] = url_for("essential")
    ...
    if request.method == "POST":
        ...
        if Email and Password:
            ...
            if response:
                ...
                return redirect(session.get("ret", url_for("root")))
            else:
...
def gitHub_login():
    return github.authorize_redirect(github_redirect_uri)
...
def login_microsoft():
    return microsoft.authorize_redirect(microsoft_redirect_uri)
...
def features_pro():
    session["ret"] = url_for("features_pro")
    if not session.get("login"):
        return redirect(url_for("Login"))
...
def ask():
    ...
    response = Client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                "You are Zyra, a premium personal AI assistant. Your tone is professional, sleek, "
                "and highly intellectual. You maintain a high-end, luxury tech vibe. Respond concisely. "
                "You are trained by Aaquib Khilji to help people. You have access to the full Folio.X website. "
                "Folio.X makes company-specific resumes and provides company-specific templates."
            )
        )
    )
...
def summary():
    ...
    response = Client.models.generate_content(
        ...
        contents=question,
        config=types.GenerateContentConfig(
            system_instruction="You are a Summary AI assistant. Give responses such that it seems like the user is presenting a summary of themselves."
        )
    )
    return jsonify({"response": response.text}), 200

# ATS Image Chunk Receiver
...
# Get/Update Chat Sessions for Zyra
@app.route("/getSessionData")
def getData():
    return jsonify(session.get("zyra", 0))
...
def Pro_resume():
    session["ret"] = url_for("Pro_resume")
